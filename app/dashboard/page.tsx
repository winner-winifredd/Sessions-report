...
const WHERE_TO_BILL_OPTIONS = [
  "Headway",
  "Lytec (Intake Only)",
  "Lytec (Follow-ups Only)",
  "Self Pay",
];
...
export default function DashboardPage() {
  ...
  const [savingUrlRowIndex, setSavingUrlRowIndex] = useState<number | null>(null);
  ...
  async function saveMissingNotesUrl(rowIndex: number, value: string) {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        setUrlSaveError(data.error || "Failed to save");
      }
    } catch (e) {
      setUrlSaveError(e instanceof Error ? e.message : String(e));
    } finally {
  ...
  const rowsForTimeCards = useMemo(
    () =>
      payPeriodFilter === "all"
        ? rows
        : rows.filter((r) => getPaymentPeriodKey(r.dateOfSession) === payPeriodFilter),
  ...
  const rootClassName =
    "min-h-screen " + (isDark ? "bg-[#0c1116] text-slate-200" : "bg-slate-50 text-slate-900");
    ...
    <RootLayout className={rootClassName}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={
                "flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition " +
                (isDark
                  ? "border border-slate-700 bg-slate-800/80 text-teal-300 hover:bg-slate-700 hover:border-teal-500/50"
                  : "border border-slate-200 bg-white text-teal-700 hover:bg-slate-100 hover:border-teal-400")
              }
            >
      ...
      <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6">
        ...
        {urlSaveError && (
          <div
            className={
              "mb-6 flex items-center justify-between gap-3 rounded-xl px-4 py-3 " +
              (isDark
                ? "border border-amber-900/50 bg-amber-950/30 text-amber-300"
                : "border border-amber-200 bg-amber-50 text-amber-800")
            }
          >
            <span>Missing Notes URL save failed: {urlSaveError}</span>
            <button
              type="button"
              onClick={() => setUrlSaveError(null)}
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card
            ...
            label="Final rows"
            value={String(
              filtered.filter((r) => String(r.finalRowFlag).toLowerCase() === "true").length,
            )}
          ...
          <>
            ...
            >
              <div className="overflow-x-auto overflow-y-visible">
                <table className="w-full min-w-[3200px] table-auto text-left text-[11px] sm:text-xs">
                  <thead>
                    ...
                    {pagedRows.map((r, i) => (
                      ...
                      >
                        {ALL_COLUMNS.map((col) => {
                          ...
                          if (col.key === "missingNotesUrl") {
                            const current = r.missingNotesUrl || "";
                            const trimmed = current.trim();
                            const rowIndex = rows.findIndex((row) => row === r);
                            const isSaving = savingUrlRowIndex === rowIndex;
                            return (
                              <Td key={col.key} className="min-w-[220px]">
                                <input
                                  type="text"
                                  value={current}
                                  onChange={(e) => {
                                    ...
                                    setRows((prev) =>
                                      prev.map((row) =>
                                        row === r ? { ...row, missingNotesUrl: next } : row,
                                      ),
                                    );
                                  }}
                                  onBlur={() => {
                                    if (rowIndex >= 0) {
                                      saveMissingNotesUrl(rowIndex, (r.missingNotesUrl || "").trim());
                                    }
                                  }}
                                  className={
                                    "mb-1 min-w-[200px] w-full max-w-full rounded border px-2 py-1 text-[10px] overflow-x-auto " +
                                    (isDark
                                      ? "border-slate-700 bg-slate-900 text-slate-100"
                                      : "border-slate-300 bg-white text-slate-900")
                                  }
                                  placeholder="Paste or edit missing-notes URL..."
                                  disabled={isSaving}
                          ...
                          if (col.key === "insurance") {
                            ...
                            return (
                              <Td key={col.key} className="min-w-[160px]">
                                <select
                                  value={current}
                                  onChange={(e) => {
                                    const next = e.target.value;
                                  ...
                                  className={
                                    "min-w-full w-full rounded border px-2 py-1 text-[11px] sm:text-xs " +
                                    (isDark
                                      ? "border-slate-700 bg-slate-900 text-slate-100"
                                      : "border-slate-300 bg-white text-slate-900")
                                  }
                          ...
                          if (col.key === "whereToBill") {
                            ...
                            return (
                              <Td key={col.key} className="min-w-[200px]">
                                <select
                                  value={current}
                                  onChange={(e) => {
                                    const next = e.target.value;
                            ...
                            return (
                              <Td key={col.key} className="min-w-[200px]">
                                <select
                                  ...
                                  className={
                                    "min-w-full rounded border px-2 py-1 text-[11px] sm:text-xs " +
                                    (isDark
                                      ? "border-slate-700 bg-slate-900 text-slate-100"
                                      : "border-slate-300 bg-white text-slate-900")
                                  }
                          ...
                          if (col.key === "dnsReason") {
                            ...
                            return (
                              <Td key={col.key} className="min-w-[180px]">
                                <select
                                  value={current}
                                  onChange={(e) => {
                                    const next = e.target.value;
                          ...
                          const wideCols = ["appointmentKey", "patientName", "note", "missingNotesAuditStatus", "icd10Codes", "dnsReason"];
                            return (
                              <Td key={col.key} className={wideCols.includes(col.key) ? "min-w-[180px]" : undefined}>
                                {value || "—"}
                              </Td>
                            );
                        })}
        ...
        {providerSummaries.length > 0 && (
          <section className="mt-8">
            ...
            >
              <div className="overflow-x-auto overflow-y-visible">
                <table className="w-full min-w-[1000px] table-auto text-left text-[11px] sm:text-xs">
                  <thead>
                    <tr
                      className={
                        isDark
                          ? "border-b border-slate-800 bg-slate-800/60"
...
function Th({ children }: { children: React.ReactNode }) {
  return (
    <th className="min-w-[110px] shrink-0 px-2 py-1 font-medium text-slate-400 whitespace-nowrap">
      {children}
    </th>
  );
}
...
function Td({ children, colSpan, className }: TdProps) {
  return (
    <td
      colSpan={colSpan}
      className={
        "min-w-[110px] shrink-0 overflow-visible px-2 py-1 align-middle whitespace-nowrap text-inherit" +
        (className ? ` ${className}` : "")
      }
    >
      {children}
